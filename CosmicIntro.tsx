  ...
  return (
    <div className="relative h-screen w-full overflow-hidden flex flex-col items-center justify-center bg-cosmos">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-20">
         {/* Placeholder for Shiva Silhouette using CSS shapes or an SVG would go here. Using a divine circle for abstraction. */}
      ...
      {/* Sound Toggle (AUM Chant Control) */}
      <button 
        onClick={() => setIsMuted(!isMuted)}
        className="absolute bottom-8 right-8 z-20 flex items-center gap-2 text-gray-400 hover:text-saffron transition-colors"
      >
        <span className="text-xs uppercase tracking-widest hidden md:block">{isMuted ? 'Enable Cosmic Sound' : 'Listening to AUM'}</span>
